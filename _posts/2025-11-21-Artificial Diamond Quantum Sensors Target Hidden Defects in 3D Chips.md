...
In the ever‑shifting landscape of chip manufacturing, a promising new player is turning heads. A startup called EuQlid (based in College Park, Maryland) is deploying quantum sensors made from artificial diamonds to uncover buried faults in next‑generation 3D semiconductor stacks — all without damage or slow processing. ([IEEE Spectrum][1])

---

### Why this matters

Semiconductor makers are increasingly moving toward stacked “3‑D” integration – layering transistors and other components vertically to pack more performance into less space. ([IEEE Spectrum][1]) But with that stacking comes risk: if a buried metal interconnect fails, or a crack forms deep in the silicon, the entire chip stack may have to be discarded. As EuQlid co‑founder and CEO Sanjive Agarwala puts it: “with the rise of 3‑D sandwich integration … you have to throw out the entire sandwich. That’s a massive, massive problem.” ([IEEE Spectrum][1])

Currently, chip defects are inspected using optical methods (which struggle with depth) or X‑ray techniques (which are slow and can damage silicon) — neither of which are ideal for high‑volume manufacturing. ([IEEE Spectrum][1])

---

### What EuQlid is offering

EuQlid’s platform, dubbed “QuMRI,” uses microscopic artificial diamonds embedded with nitrogen‑vacancy (NV) centers. These defects in the diamond lattice respond to minute magnetic and electric field changes caused by current flow in buried interconnects. ([IEEE Spectrum][1]) In practice, when illuminated with green light, the NV centers fluoresce red — and that fluorescence alters in response to nearby currents. By mapping that fluorescence, the system can detect hidden faults **non‑invasively**. ([IEEE Spectrum][1])

According to Agarwala, the system can inspect to depths of around 100–150 micrometres — enough for many stacked silicon layers — and claims to be about 100× faster than X‑ray inspection while avoiding damage. ([IEEE Spectrum][1])

---

### Broader implications & challenges

If this works at scale, EuQlid’s technology could cut costs by lowering scrap rates in advanced fabs, speeding inspection throughput, and enabling deeper integration of 3‑D chip designs. One potential additional use: hardware‑trojan detection. Agarwala suggests that if malicious current flows occur in a chip, the sensor could pick them up. ([IEEE Spectrum][1])
...
* **Nitrogen‑vacancy (NV) center**: A specific defect in a diamond’s carbon lattice where a nitrogen atom replaces a carbon atom and an adjacent carbon atom is missing. NV centers are sensitive to magnetic/electric fields and can be used for quantum sensing. ([IEEE Spectrum][1])
...
Source link: [https://spectrum.ieee.org/quantum-sensors-2674296517](https://spectrum.ieee.org/quantum-sensors-2674296517)

[1]: https://spectrum.ieee.org/quantum-sensors-2674296517 "Quantum Sensor Startup Seeks Flaws in 3D Chips - IEEE Spectrum"
