...
As a personal developer, we do not have sufficient budget to support. Fortunatelly there are a few platform provding free resources to deploy service. After a few survey, I select [Vecel](https://vercel.com) to deploy frontend flutter (Severless), and [Railway](https://railway.com/) to deploy backend API. Both support Github CI integrattion. So you just update the repos in Github, which trigger updates in your frontend and backend services.
